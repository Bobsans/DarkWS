...
These methods require a matching server with `darkws:authenticate` and `darkws:logout`
support. Update the application-owned token/query source on logout so reconnect
does not restore stale credentials.
